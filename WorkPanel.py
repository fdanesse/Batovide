...
from gtksourceview2 import CompletionProvider
import pango
from gtk import gdk
...
class WorkPanel(gtk.VPaned):
    ...
    def __init__(self):
        ...
        self.notebook_sourceview.connect(
            'new_select', self.__re_emit_new_select)
        self.terminal.connect(
            "ejecucion", self.__set_ejecucion)
        self.terminal.connect(
            "reset", self.detener_ejecucion)
    ...
    def ejecutar(self, archivo=None):
        ...
        if not archivo or archivo == None:
            ...
            # No debiera estar activo el botón ejecutar.
            if not pagina > -1:
                return
...
class Notebook_SourceView(gtk.Notebook):
    ...
    def __switch_page(self, notebook, page, page_num):
        ...
        scrolled = self.get_children()[page_num]
        view = scrolled.get_children()[0]
    ...
    def __cerrar(self, widget):
        ...
        for indice in range(paginas):
            ...
            if boton == widget:
                self.get_children()[indice].get_child().set_accion(
                    "Cerrar Archivo")
                break
...
class SourceView(gtksourceview2.View):
    ...
    def set_formato(self, fuente, tamanio):
        ...
        self.modify_font(
            pango.FontDescription("%s %s" % (fuente, tamanio)))
            ...
            if self.archivo:
                if os.path.exists(self.archivo):
                    if self.control:
                        if self.control != os.path.getmtime(self.archivo):
                            dialogo = gtk.Dialog(
                                ...
                                flags=gtk.DIALOG_MODAL,
                                buttons=(
                                    "Recargar", gtk.RESPONSE_ACCEPT,
                                    "Continuar sin recargar", gtk.RESPONSE_CANCEL))

                            dialogo.set_border_width(15)

                            label = gtk.Label(
                                "El archivo ha sido modificado por otra aplicación.")
                            label.show()
                            ...
                            elif gtk.ResponseType(response) == gtk.RESPONSE_CANCEL:
                                return False
                    ...
                    dialogo = gtk.Dialog(
                        ...
                        flags=gtk.DIALOG_MODAL,
                        buttons=(
                            "Guardar", gtk.RESPONSE_ACCEPT,
                            "Continuar sin guardar", gtk.RESPONSE_CANCEL))

                    dialogo.set_border_width(15)

                    label = gtk.Label(
                        "El archivo fue eliminado o\n movido de lugar por otra aplicación.")
                    label.show()
...
class AutoCompletado(gobject.GObject, CompletionProvider):
    ...
    '''
    def do_activate_proposal(self, comp_item, text_iter):
        """
        Cuando se selecciona y clickea
        una posible solución.
        """

        self.buffer.insert(text_iter, comp_item.props.label)
        ...
        if texto_de_linea_en_edicion.endswith("."):
            ...
            if expresion:
                ...
                for item in lista:
                    ...
                    opciones.append(gtksourceview2.CompletionItem(
                        item, item))
